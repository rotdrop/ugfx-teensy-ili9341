...
namespace CJHTeensy {
    ...
    : FTMOverflow<0>
  {
   protected:
    ...
    static constexpr uint8_t prescaler_ = intLog2(clock_ / (1U << 16) / frequency_);
    static constexpr uint32_t tick_ = clock_ >> prescaler_;
    static constexpr uint16_t overflow_ = (tick_ + frequency_ - 1) / frequency_;

    static constexpr uint16_t tickMin_ =     deciUsToTicks(tick_, 7500);
    static constexpr uint16_t tickMax_ =    deciUsToTicks(tick_, 22500);
    static constexpr uint16_t tickCenter_ = deciUsToTicks(tick_, 15000);

   public:
    ServoController(uint64_t pinMask = ~0UL)
    {
      for (int i = 0; i < FTMModule::numInstances; ++i) {
        ...
        FTMModule::instance(i).SC = FTM_SC_CLKS(1) | FTM_SC_PS(prescaler_);
        ...
        for (int pin = 0; pin < FTMModule::numChannels[i]; ++pin) {
          if (pinMask & (1 << FTMModule::pins[i][pin])) {
        ...
        FTMModule::instance(i).PWMLOAD = channelMask;
